...
public class JuegoProgramacion extends JFrame{
    ...
    public JuegoProgramacion() {
        ...
        Image heal, pj, monstruo, tesoro, muro, puerta;

        try {
            heal = ImageIO.read(new File(path + "\\src\\juego\\imagenes\\heal.png"));
            pj = ImageIO.read(new File(path + "\\src\\juego\\imagenes\\pj.png"));
            monstruo = ImageIO.read(new File(path + "\\src\\juego\\imagenes\\monstruo.png"));
            tesoro = ImageIO.read(new File(path + "\\src\\juego\\imagenes\\tesoro.png"));
            muro = ImageIO.read(new File(path + "\\src\\juego\\imagenes\\muro.png"));
            puerta = ImageIO.read(new File(path + "\\src\\juego\\imagenes\\door.png"));
        } catch (IOException e) {
            ...
            public void paint(Graphics g){
                ...
                for (int i = 0; i < mapa.length; i++) {
                    for (int j = 0; j < mapa[i].length; j++) {
                        ...
                        switch (mapa[i][j]) {
                            case '#' -> g.drawImage(muro, x, y, width, height, null);
                            case 'P' -> g.drawImage(pj, x, y, width, height, null);
                            case 'E' -> g.drawImage(monstruo, x, y, width, height, null);
                            case 'T' -> g.drawImage(tesoro, x, y, width, height, null);
                            case 'D' -> g.drawImage(puerta, x, y, width, height, null);
                            case 'H' -> g.drawImage(heal, x, y, width, height, null);
                        }
                        x += width;
                    }
                    x = 0;
